...
function LateUpdate () {
	...
	// Remove old sections
	while (sections.length > 0 && now > sectionTronTrail.time + time) {
		sections.Pop();
	}
	
	sectionTronTrail = sections[0];
	// Add a new trail section
	if (sections.length == 0 || (sectionTronTrail.point - position).sqrMagnitude > minDistance * minDistance)
	{
